    ...
    try {
        ...
        if (response.ok) {
            ...
            if (result === null || result === undefined) {
                return `error: ${config.alistUserName} 获取 token 失败`;
            }
...
async function redirect2Pan(r) {
    ...
    // var alistToken = await getAlistToken();
    const alistToken = config.alistToken;
    r.warn(`当前的AlistToke: ${alistToken}`);
    ...
    if (!alistRes.startsWith("error")) {
        alistRes = alistRes.includes("http://192.168.50.2")
            ? alistRes.replace("http://192.168.50.2", alistPublicAddr)
            : alistRes;
    ...
    if (alistRes.startsWith("error500")) {
        ...
        for (let i = 0; i < folders.length; i++) {
            ...
            if (!driverRes.startsWith("error")) {
                driverRes = driverRes.includes("http://192.168.50.2")
                    ? driverRes.replace("http://192.168.50.2", alistPublicAddr)
                    : driverRes;
