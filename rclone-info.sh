...
docker run -d --name rclone-alist -v /etc/passwd:/etc/passwd:ro -v /etc/group:/etc/group:ro -v /data/videos/tools/rclone/config:/config/rclone -v /data/videos/media/alist:/data:shared -v /data/videos/tools/rclone/cache:/cache --device /dev/fuse:/dev/fuse --cap-add SYS_ADMIN --security-opt apparmor:unconfined --restart always rclone/rclone mount alist_webdav:/ /data --cache-dir /cache --allow-other --vfs-cache-mode writes --allow-non-empty
